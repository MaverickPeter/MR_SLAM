      ...
      visibilityCleanupDuration_(0.0)
{
  ...
  // frontierPublisher_ = nodeHandle_.advertise<sensor_msgs::Image>("/" + robot_name + "/frontier", 1);
}
                                          ...
                                          float *var, int *point_colorR, int *point_colorG, int *point_colorB, float *rough, float *slope, float *traver, float* intensity)
{
  ...
  for (GridMapIterator iterator(visualMap_); !iterator.isPastEnd(); ++iterator) {
    ...
    if(position.x() < 2.0 && position.x() > -2.0 && position.y() < 2.0 && position.y() > -2.0){
      visualMap_.at("elevation", *iterator) = 0.0;
      visualMap_.at("traver", *iterator) = 1.0;
      show_point.x = position.x();
      show_point.y = position.y();
      show_point.z = 0.0;
      show_point.r = 0;
      show_point.g = 0;
      show_point.b = 0;
      ...
      setInner = true;
      // cout << "inner point: " << position.x() << " y: " << position.y() << endl;
    }

    if(elevation[index] != -10 && !setInner && traver[index] != -10 && !std::isnan(traver[index]))
    // if(!std::isnan(traver[index]) || !std::isnan(elevation[index]))
    {
  ...
  sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", frontierImage).toImageMsg();
  // frontierPublisher_.publish(msg);
  ...
  show_pointCloud.header.frame_id = robot_name + "/odom";
...
grid_map::GridMap ElevationMap::findFrontiers(grid_map::GridMap& gridMap, int length) 
{
  ...
  //       int yy = j + dy[k];
  //       if(xx >= 0 && yy >= 0 && xx < length && yy < length){
  //         if((dilatedMap[i][j] == 0) && (dilatedMap[xx][yy] == -1))
  //           count ++;
  //       }
  //     }
  //     if(count > 2 && !(i == 0 || j == 0 || i == length || j == length)){
  //       floodMap[i][j] = 2;
  //     }else if(count >= 1 && (i == 0 || j == 0 || i == length || j == length)){
  //       floodMap[i][j] = 2;
  //     }
  //   }
  // }
