...
def load_pc_infer(pc):
    ...
    # hits = pc[np.where((np.abs(pc[:,0]) < 70.)&(np.abs(pc[:,1]) < 70.)&(np.abs(pc[:,2]) < 20.)&(pc[:,2] > -2.)&(np.abs(pc[:,0]) > 5.)&(np.abs(pc[:,1]) > 5.))]
    # hits[...,0] = hits[...,0] / 70.
    # hits[...,1] = hits[...,1] / 70.
    # hits[...,2] = (hits[...,2] + 2) / 20.
...
def load_pc_file(filename):
    ...
    #transer = gpuadder.GPUTransformer(pc, size, cfg.max_length, cfg.num_ring, cfg.num_sector, cfg.num_height, cfg.overlap_num)
    #transer.transform()
    #point_t = transer.retreive()
    #point_t = point_t.reshape(-1, 3)
    #point_t = point_t[...,2]
